...
require ('..\script\Database.php')  ;
 ...
 $db_conn = getDb();
 $query = "SELECT * FROM `film` WHERE id=3  ";
 ...
 while($record = mysqli_fetch_assoc($result_set)){
	 echo "<tr> <td> {$record['id']} </td>
                  ...
                  <td> {$record['jaar']} </td>
                  <td> {$record['titel']} </td>
                  <td> <a href=edit_prooi.php?id=".$record['id']."> Edit Jaar </a> </td>
				  </tr>";
...
<center>
<img src="/project/afbeeldingen/prooi.png" width="300" height="500" title="Prooi cover" alt="Prooi" />
</center> 
<br/>
<h2>
Mooie citaten: 
...
<?php
$db_conn = getDb();
 $query1 = "SELECT * FROM `citaat` WHERE idfilm=3  ";
 $result_set = mysqli_query($db_conn, $query1);
 while($record = mysqli_fetch_assoc($result_set)){
	 echo "<br> {$record['citaat']} <br>
    <a href=delete_p.php?id=".$record['id']."> Delete </a> <br> <tr>" ;
 }
?>

<form method="POST" action='process_prooi.php'> <br>
...
De regisseur is: <?php
$db_conn = getDb();
 $query2 = "SELECT * FROM `regisseur` WHERE id=3  ";
...
<h3>
<a href="https://www.youtube.com/watch?v=xR7BIp_u4AA">  Kijk hier de trailer!</a>
</h3>
</body>
